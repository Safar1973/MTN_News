const translations = {
    ...
    de: {
        ...
        checkoutTitle: "Kasse",
        checkoutInfo:
            "Bitte füllen Sie Ihre Daten aus und wählen Sie die gewünschte Zahlungsmethode.",
    ...
    ar: {
        ...
        paymentOnline: "دفع أونلاين (بطاقة / بايبال)",
        paymentCash: "الدفع عند الاستلام",
        clearCart: "تفريغ السلة",
        checkout: "إتمام الشراء",
        homeTitle: "مرحباً بك في متين فود ماركت",
        homeSubtitle: "نوفر لك منتجات غذائية طازجة بجودة عالية إلى باب منزلك.",
        homeShopBtn: "اذهب إلى المنتجات",
        headerSloganTitle: "مرحباً بك في متين فود Gmbh",
        ...
        checkoutInfo:
            "يرجى تعبئة البيانات واختيار طريقة الدفع المناسبة لك.",
        formName: "الاسم الكامل",
        formAddress: "العنوان",
        formCity: "المدينة",
        formPhone: "رقم الهاتف",
        formPaymentSelect: "طريقة الدفع",
        optOnline: "دفع أونلاين (بطاقة / بايبال)",
        optCard: "الدفع بالبطاقة عند الاستلام",
        optCash: "الدفع نقداً عند الاستلام",
        placeOrder: "تأكيد الطلب",
        backHome: "العودة للرئيسية",
        backProducts: "العودة للمنتجات",
        orderSummary: "ملخص الطلب",
        orderTotalLabel: "المجموع:",
        orderPlacedMsg: "تم تسجيل طلبك (تجريبي).",
        cats: {
            all: "جميع المنتجات",
            grains: "الحبوب والبقوليات",
            syrups: "الدبس والمربيات",
            honey: "العسل والزعتر",
            dairy: "الألبان والأجبان",
            meat: "اللحوم والدواجن",
            drinks: "القهوة والمشروبات",
            canned: "المعلبات",
...
/* تغيير اللغة وتحديث كل النصوص */
function setLanguage(lang) {
    currentLang = lang;
    const t = translations[lang];

    // Update direction (RTL للغة العربية)
    ...
    renderProducts(); // تحديث المنتجات بالاسم والعملات
    updateCartUI(); // تحديث نصوص السلة
    updateOrderSummary();
}

/* تفعيل رابط الكاتيجوري */
...
/* زيادة / إنقاص كمية منتج في السلة */
function changeQty(productId, delta) {
    ...
    item.qty += delta;
    if (item.qty <= 0) {
        // حذف المنتج بالكامل لو أصبحت الكمية 0
        ...
        row.innerHTML = `
            <div class="cart-item-info">
                ...
                <div class="cart-item-controls">
                    ...
                    <button class="qty-btn" onclick="changeQty(${item.product.id}, 1)">+</button>
                    <button class="remove-item-btn" onclick="removeItem(${item.product.id})">🗑</button>
                </div>
            </div>
...
/* فتح صفحة الدفع من زر Checkout */
function openCheckout() {
    // لو السلة فارغة لا نذهب إلى الدفع
    if (cart.length === 0) {
